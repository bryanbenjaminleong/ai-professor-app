...
export async function scrapeArsTechnica(
  sourceUrl: string,
  ...
  try {
    ...
    $('article, [class*="post"], [class*="article"]').each((index, element) => {
      ...
      try {
        ...
        items.push({
          ...
          image_url: null,
          author: author || null,
          tags: ['ai', 'tech', 'news', 'ars-technica'],
        })
      } catch (error) {
    ...
    errors.push({ url: sourceUrl, error: `Failed to scrape Ars Technica: ${error}` })
  }
