...
export async function scrapeStabilityAI(
  sourceUrl: string,
  ...
  try {
    ...
    $('article, [class*="news"], [class*="post"]').each((index, element) => {
      ...
      try {
        ...
        items.push({
          ...
          image_url: null,
          author: null,
          tags: ['stability-ai', 'stable-diffusion', 'ai', 'announcements'],
        })
      } catch (error) {
    ...
    errors.push({ url: sourceUrl, error: `Failed to scrape Stability AI: ${error}` })
  }
